	...
	switch (process) {
		case 'add-anime':
			try {
				...
				await mariadb(`INSERT INTO log (user, text, process_type, process) VALUES ("${username}", "${text.replace(/([!@#$%^&*()+=\[\]\\';,./{}|":<>?~_-])/g, "\\$1")}", "${process}", "success")`)
			} catch (err) {
				console.log(`Loglara kayıt ederken hata.\nOlay yeri: ${process}\nSorunu çıkaran girdi: ${documentId}`)
			}
			break;
		case 'update-anime':
			try {
		...
		case 'add-episode':
			try {
				...
				mariadb(`INSERT INTO log (user, text, process_type, process) VALUES ("${username}", "${text.replace(/([!@#$%^&*()+=\[\]\\';,./{}|":<>?~_-])/g, "\\$1")}", "${process}", "success")`)
			} catch (err) {
				console.log(`Loglara kayıt ederken hata.\nOlay yeri: ${process}\nSorunu çıkaran girdi: ${documentId}`)
			}
			break;
