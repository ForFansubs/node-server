...
// @access  Public
router.get(
    "/manga/:slug/oku/:episode_number/:filename",
    MangaEpisodeImageLimiter,
    (req, res) => {
        const { slug, episode_number, filename } = req.params;

        const path = Path.resolve(
            __dirname,
            `../../images/manga_episodes/${sanitize(slug)}/${sanitize(
                episode_number
            )}`,
            `${sanitize(filename)}`
        );
...
router.get("/metadata/:type/:slug", VariousImageLimiter, async (req, res) => {
    ...
    const path = Path.resolve(
        __dirname,
        `../../images/metadata/${sanitize(type)}/${sanitize(slug)}.png`
    );
    res.sendFile(path, async (err) => {
        if (err) {
            await CreateMetacontentCanvas({ slug, type, t: req.t });
            res.sendFile(path, (err) => {
                if (err) {
                    res.status(404).end();
                }
